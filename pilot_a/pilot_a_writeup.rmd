...
author: "Erik Santoro"
date: "`r format(Sys.time(), '%November %22, %2017')`"
output:
  html_document:
    toc: yes
    toc_depth: 3
...
#Option 1 -- When have more data points, need to verify that this computes confidence interval

d.ci <- summarySE(data=d.wide, measurevar = "numtrials", groupvars = c("task", "rolemodel"))

d.ci$rolemodel <- as.factor(d.ci$rolemodel)
d.ci$task <- as.factor(d.ci$task)
d.ci$rolemodel <- factor(d.ci$rolemodel, levels=c("past", "future")) #re-orders past & future for plot
...
ggplot(data = d.ci,
       aes(x = task, y = numtrials, fill = rolemodel)) +
  geom_bar(stat="identity", width = 0.5, position = "dodge") +
  #geom_errorbar(aes(ymin=numtrials-ci,
                    #ymax=numtrials+ci), width=.1) + #when have more data and error bars, this will plot 95% confidence intervals
...
#plotting error bars: http://www.cookbook-r.com/Graphs/Plotting_means_and_error_bars_(ggplot2)/
