...
date: "`r format(Sys.time(), '%November %23, %2017')`"
output:
  html_document:
    toc: yes
    toc_depth: 3
...
There are two confirmatory replicaiton analyses. The *key analysis of interest* is to replicate the significant interaction by task and role model, such that participants completed more trials in the fun task when role modeling their past self and completed more trials in the self-control task when role modeling their future self. 

The second confirmatory analysis is to replicate the finding that the interaction was still signficant after adding the three covariates (difficulty imainging self, the level of detail of the self, and the distance from the self). 

Importantly, as noted earlier, I analyze the results using a two-way ANOVA. I believe this is the correct statistical test to use (not a MANOVA, as the original paper used) as the goal of the analysis is to look at the interaction between the two categorical factors, task (fun or self-control) and role model (past or future self), on the number of trials completed (see here for reference: http://www.statsmakemecry.com/smmctheblog/stats-soup-anova-ancova-manova-mancova).
...
3) Other changes
    ...
    * I fixed several typos, including: (a) "your" not "you're" on page 1, (b) your job is [to] complete a "fun" task, 


**Analysis plan differences**

As noted in the "Analysis Plan" section, I conducted a two-way ANOVA rather than a univariate MANOVA as MANOVA's are for two dependent variables typically and there is only one dependent variable.
...
I created a wide data set, to be used in the anova
...
I created a wide data set, to be used in the ggplot
...
d.summarized$rolemodel <- factor(d.summarized$rolemodel, levels=c("past", "future")) #re-orders past & future for plot
...
ggplot(data = d.summarized,
  ...
  coord_cartesian(ylim=c(1,8)) +                                      #makes limit of 10
  labs(x = "Task Frame", 
...
Exploratory analysis 2: Test main confirmatory analysis with various with
