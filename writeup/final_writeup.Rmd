...
```
  ...
  This analysis confirmed what we saw in the chart: the interaction was not significant. Moreover, as we saw, there was a main effect by task. Even though the interaction was not signficant, and seemed to be in the opposite direction, let's look at the effect size (eta squared).
...
```
 ...
 The interaction is not significant, again, using a null hypothesis significant testing approach, showing that the replication failed.
...
```
  ...
  Using the 95% confidence intervals as approximation, there does not appear to be an interaction of task and self. Again, we observe in the fun task, the opposite, albeit nonsignificant, trend such that participants motivated by their future self completed more trials than participants motivated by their past self.
...
####Exploratory Analysis #4 -- Time differences by condition
  ...
  Thus, I want to know: how much time were participants spending on each page before submitting? Below is a histogram of the average time spent per trial.
...
```
  ...
  This data set looks now nearly normally distributed. Thus, let's use a two-way ANOVA on the log transofrmmed data. Let's also still plot the average time spent per page per condtion just to see differences.
...
```
  ...
  The interaction is not signficant, per the two way anova. Looking visually at the raw means, it does seem to be the case that participants role modeling their future self spent more time in both tasks, particularly in the self-control task (not-signficant but trending).
