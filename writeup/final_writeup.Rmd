...
The original authors used a MANOVA when looking at one outcome variable (the number of trials completed) and two factors each with two levels (role model -- past or future self; task -- fun or self-control). For simplicity purposes, I used a two-way ANOVA; note that these should be equivalent.
...
As noted earlier, I plan to analyze the results using a two-way ANOVA, which should be approximately equivalent to using a MANOVA.
...
As noted in the "Analysis Plan" section, I conduct a two-way ANOVA rather than a univariate MANOVA.
...
d.tidy$difficult <- as.numeric(d.tidy$difficult)
d.tidy$detailed <- as.numeric(d.tidy$detailed)
d.tidy$distancenow <- as.numeric(d.tidy$distancenow)
d.tidy$distancetask <- as.numeric(d.tidy$distancetask)
...
```
  ...
  Since this is count data, let's model the interaction with poisson regression. Here are the results of the poisson model:
